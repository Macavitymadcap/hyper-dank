...
const lcov = `TN:
SF:${path.join(root, "apps/walking-pace/src/app.tsx")}
FNF:2
FNH:2
LF:10
LH:8
end_of_record
SF:${path.join(root, "libs/components/src/atoms/Button/Button.tsx")}
FNF:3
FNH:3
LF:5
LH:5
...
describe("coverage report", () => {
  ...
  test("renders a browsable html summary", () => {
    ...
    expect(html).toContain("apps/walking-pace/src/app.tsx");
  });
    ...
    expect(report.thresholdFailures({ functions: 1, lines: 0.9 })).toEqual([
      {
        ...
        expected: 0.9,
        metric: "lines",
        path: "apps/walking-pace/src/app.tsx",
      },
    ]);
