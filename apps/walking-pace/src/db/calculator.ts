const getSpeed = (miles: number, minutes: number, seconds: number): number => {
  if (miles <= 0 || (minutes <= 0 && seconds <= 0)) return 0;
  const totalHours = minutes / 60 + seconds / 3600;
  return miles / totalHours;
};

const getPace = (miles: number, minutes: number, seconds: number): number => {
  if (miles <= 0) return 0;
...
const getAverage = (numbers: number[]): number => {
  ...
  const average = total > 0 ? numbers.reduce((a, b) => a + b, 0) / total : 0;
