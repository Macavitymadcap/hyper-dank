...
import { LOCAL_DEV_PASSWORD } from "../../../apps/walking-pace/src/envs/local/local-presets";
...
test("opens the Storybook About guide from the app header", async ({ page }) => {
  await signIn(page, userEmail);
  await page.goto("/");
  await page.getByRole("link", { name: "About" }).click();

  await expect(page).toHaveURL(/\/storybook\/\?path=\/story\/guides-about--about/);
  await expect(
    page.frameLocator("#storybook-preview-iframe").getByRole("heading", {
      name: "About Hyper-Dank",
    }),
  ).toBeVisible();
});
