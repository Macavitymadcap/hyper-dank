...
- Do not add a starter CLI, template repository, npm publishing automation, or external package
  release workflow in this epic. Capture those as follow-up candidates after the docs tell the
  current truth.
...
1. **Adopter trust**: state the package status plainly, show what is stable, and document limits
   before readers have to infer them from source.
2. **Reference depth**: give each package a proper public contract, including exported types,
   helper behaviour, examples, errors, CSS hooks, and accessibility expectations where applicable.
3. **Navigation clarity**: make docs and Storybook feel like two connected surfaces: docs for
   framework and API guidance, Storybook for rendered UI behaviour.
4. **Reviewability**: add checks that keep the public docs from drifting away from package exports,
   Storybook stories, recipes, and README claims.
5. **Positioning**: explain the value of server-rendered JSX, Hono, HTMX, Bun scripts, and shared
   package boundaries in practical terms instead of generic productivity language.
6. **GitHub-native planning**: make GitHub Issues and Projects the operational system for tickets,
...
| `pace-0062` | `pace-0062` | Clarify package installation, consumption, and README contracts | Root and package READMEs explain current install/pack/GitHub or npm status, imports, CSS/subpath exports, peer dependencies, package limits, and links to docs/Storybook |
...
- `README.md` and package READMEs answer installation, package status, imports, compatibility, and
  next-link questions.
...
- `pace-0013` through `pace-0016` are intentionally in the separate `pace-0013` branch and PR #78,
  not in this branch yet. They should be treated as active parallel work rather than missing work.
...
- Publish packages to npm or document a deliberate GitHub/package-tarball consumption route with
  release automation.
