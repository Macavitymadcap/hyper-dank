...
building with JSX, Hono, Bun, and HTMX. The work should explain why Hyper-Dank exists, how Campaign
Ledger or another app can install or consume its libraries, which examples are reusable, and what
each public component or helper guarantees. It should also make the public copy sell the advantages
of Hyper-Dank without turning the docs into vague marketing material.
...
- Establish a real installation path for Campaign Ledger before implementation work depends on
...
- Do not build broad release automation unless the selected Campaign Ledger installation route
...
1. **Installable packages**: make Hyper-Dank consumable from Campaign Ledger through a tested,
...
| `pace-0062` | `pace-0062` | Establish package installation, distribution, and README contracts | Campaign Ledger has a tested way to consume Hyper-Dank packages; root and package READMEs explain current npm/GitHub/package-tarball status, imports, CSS/subpath exports, peer dependencies, package limits, and links to docs/Storybook |
...
- `bun run test:compat`
- `bun run verify`
- Test the selected Campaign Ledger installation route from outside the Hyper-Dank workspace, using
  public package names and the same package manager flow Campaign Ledger will use.
...
- Campaign Ledger has a tested installation path for all Hyper-Dank packages it needs before it
