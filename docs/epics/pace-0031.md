# pace-0031: Broaden Hyper-Dank Platform Primitives
...
| Ticket Branches | `pace-0032`, `pace-0033`, `pace-0034`, `pace-0035`, `pace-0036`, `pace-0037`, `pace-0038` |
| Owner | `@Macavitymadcap` |
| Target PR | [#43](https://github.com/Macavitymadcap/pace-calculator/pull/43) |
...
- Broaden `@macavitymadcap/hyper-dank-ui` with reusable primitives for document/layout,
...
- Document reusable app-shape recipes for static sites, dashboards, server apps, and static demos.
...
| --- | --- | --- | --- |
| `pace-0032` | `pace-0032` | Audit reusable needs across Walking Pace and Character Sheet | Inventory current app components and scripts, identify shared candidates, and update the epic if the ticket map needs adjustment |
| `pace-0033` | `pace-0033` | Add broader shared component primitives | Shared components cover reusable layout/navigation/data/form/feedback gaps with tests, exports, CSS contracts, and Storybook coverage |
| `pace-0034` | `pace-0034` | Introduce `@macavitymadcap/hyper-dank-automation` | A new package exports process, GitHub, verification, local-server, screenshot, and browser-test helpers with package docs and tests |
| `pace-0035` | `pace-0035` | Migrate this repo's scripts onto the shared script package | Walking Pace scripts use the shared package where appropriate while preserving current command behaviour and verification output |
| `pace-0036` | `pace-0036` | Expand consumer compatibility for Character Sheet and generic app shapes | Compatibility tests cover Character Sheet-style UI plus blog, dashboard, and static/demo component compositions through public package imports |
| `pace-0037` | `pace-0037` | Document platform adoption patterns | Public docs and package READMEs explain component groups, script-library usage, and app-shape recipes for server apps, static demos, blogs, and dashboards without consuming-app handoff content |
| `pace-0038` | `pace-0038` | Rename public package identities | Package names, imports, docs, compatibility fixtures, and release notes use the new naming convention with an explicit breaking-change note |
...
placeholders until `pace-0038` records the final convention and updates `package.json` names,
...
```mermaid
flowchart LR
  Components["@macavitymadcap/hyper-dank-ui"] --> Apps["Hyper-Dank apps"]
  Scripts["@macavitymadcap/hyper-dank-automation"] --> Apps
  Database["@macavitymadcap/hyper-dank-data"] --> Apps
  Http["@macavitymadcap/hyper-dank-transport"] --> Apps
...
`@macavitymadcap/hyper-dank-ui/styles.css` and layer app-specific CSS afterwards.
...
- Branch each ticket from `pace-0031`.
- Merge completed ticket branches back into `pace-0031`.
...
- The next epic number is `pace-0031`.
