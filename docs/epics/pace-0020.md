...
- Keep `@macavitymadcap/hyper-dank-ui`, `@macavitymadcap/hyper-dank-data`, and
  `@macavitymadcap/hyper-dank-transport` stable for external consumers.
...
`@macavitymadcap/hyper-dank-ui` remains a reusable Hono JSX package. Shared UI changes should
...
- After merge to `main`, manually rename the GitHub repository from `pace-calculator` to
  `hyper-dank`.
- The Pages workflow derives the project base path from the repository name, so a pre-rename deploy
  uses `/pace-calculator` and a post-rename deploy uses `/hyper-dank`; rerun the workflow after the
  rename to publish the stable path.
...
- Repository-path Pages hosting should work during the rename transition and under `/hyper-dank`
