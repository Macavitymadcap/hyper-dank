...
| Character Sheet `sheet-0031` branch | HTMX edit fragments, dice result placement, seeded Mira note, and final hosted-rehearsal density decision | Current best evidence for player-facing local patterns. |
...
| `pace-0051` | `@macavitymadcap/hyper-dank-automation` | Add role-aware screenshot and a11y target catalogue helpers, including optional seed/prepare callbacks and fixture asset preparation. | Keep target definitions app-local; shared tests should exercise light/dark screenshots, Pa11y cookies, and public/authenticated role targets without naming Walking Pace or Character Sheet routes. |
