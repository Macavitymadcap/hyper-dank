...
- Capture the 2026-05-24 finding that local package versions can be ahead of npm `latest`.
...
- If npm reports `E409 Conflict` for a package version, run `npm stage list <package>` before
  rerunning the workflow. The version may already be staged but not yet approved.
