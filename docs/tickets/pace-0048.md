...
- The current demo invite message deliberately uses the existing inline form feedback pattern to
  keep this ticket scoped. A later UI-system ticket should replace ad hoc `.form-error` and
  `.form-notice` usage with a shared notification/toast region that has a stable page location,
  supports stacked success/info/error messages, and lets reviewers dismiss non-critical notices.
