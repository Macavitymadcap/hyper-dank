...
- Added a Bun docs builder in `apps/walking-pace/scripts/lib/docs-build.ts` with coverage for front
  matter, Markdown tables, code blocks, raw HTML blocks, pretty routes, assets, and Pages-relative
  links.
- Rewired `prepare:pages-source` and `build:pages-assets` so `.cache/pages/site` is produced without
  `actions/jekyll-build-pages`.
- Simplified `.github/workflows/deploy.yml` to upload the repo-built Pages artifact directly.
- Updated README, architecture, and Storybook docs so Jekyll is no longer described as the active
  public docs build.
